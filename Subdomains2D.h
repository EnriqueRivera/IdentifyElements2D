...
namespace subdomains_2D
{
	 ...
	 * @class	Subdomains2D
	 *
	 * @brief	Clase que obtiene todas las áreas formadas en el CAD 2D.
	 *
	...
	class Subdomains2D 
	{
		private:
			...
			static void elements2DToPolygons(Elements2D elements, std::vector<PolyLevel> &polygonsVector, const long long scale/*, std::ofstream &myfile*/)
			{
				...
				for (size_t i = 0; i < size; i++)
				{
					polygon2DToPolygons(elements.getComps()[i]->getPolyApproach(), polygonsVector, scale);
				}
		...
		public:
			...
			/**********************************************************************************************//**
			 * @fn	static std::vector<Polygon2D*> Subdomains2D::obtainSubdomains(Elements2D elements)
			 *
			 * @brief	Obtenemos todas las áreas formadas en el CAD.
			 *
			 * @author	Enrique Alonso
			 * @date	28/01/2014
			 *
