...
namespace total_points_2D
{
	 ...
	 * @class	TotalPoints2D
	 *
	 * @brief	Clase que obtiene todos los puntos existentes en el CAD 2D.
	 *
	...
	class TotalPoints2D 
	{
		private:
			...
			static void obtainPointsComposed(Composed2D* comp, std::vector<Point2D*> &points)
			{
				std::vector<CompositeFigure*> compositeAux = comp->getcompositeFigures();
				size_t size = compositeAux.size();
				for (size_t i = 0; i < size; i++)
				{
					if (compositeAux[i]->seg)
					{
						obtainPointsSegments(compositeAux[i]->seg, points);
					}
					else if (compositeAux[i]->bez)
					{
						obtainPointsBezier(compositeAux[i]->bez, points);
					}
				}
			}
			...
			static void obtainPointsAndPolys(Elements2D elements, std::vector<Point2D*> &points, std::vector<Polygon2D*> &polys)
			{
				...
				size = elements.getCompsCount();
				for (size_t i = 0; i < size; i++)
				{
					obtainPointsComposed(elements.getComps()[i], points);
					polys.push_back(elements.getComps()[i]->getPolyApproach());
				}
		...
		public:
			...
			/**********************************************************************************************//**
			 * @fn	static std::vector<Point2D*> TotalPoints2D::obtainTotalPoints(Elements2D elements)
			 *
			 * @brief	Obtenemos el total de puntos en el CAD.
			 *
			 * @author	Enrique Alonso
			 * @date	27/01/2014
			 *
